...
You can play multiple sounds by separating them with commas. Example: lose.ogg,burn(1,3).ogg
The alert sound, alert_large for positive events and alert_small for negative events, is always played automatically.
...
Triggers a yes or no confirmation prompt when the bet meets or exceeds the threshold. Set use_percent to false to treat the amount as a flat value. Set use_percent to true to treat the amount as a percentage of what the player currently holds, so 25:true prompts when the bet is 25 percent or more of their current stat. Set the amount to 0 to disable the prompt entirely.
